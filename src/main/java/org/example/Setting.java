...
public class Setting {
    ...
    public static void loadConfig() {
        ...
        Main.PORT = Integer.parseInt(prop.getProperty("PORT", "8080"));
        String modeProp = prop.getProperty("MODE");
        if (modeProp != null && !modeProp.isBlank()) {
            Main.MODE = modeProp.trim();
        } else {
            // 向后兼容：未显式设置 MODE 时，AUTO_PLAY=true 等价于 batch
            Main.MODE = Main.AUTO_PLAY ? "batch" : "interactive";
        }
    }
    ...
    public static void saveConfig() {
        ...
        prop.setProperty("PORT", String.valueOf(Main.PORT));
        prop.setProperty("MODE", Main.MODE);
