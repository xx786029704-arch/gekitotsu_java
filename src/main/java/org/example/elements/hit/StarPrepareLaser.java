...
public class StarPrepareLaser extends org.example.Shape {     //星玉视觉激光类
    ...
    public StarPrepareLaser(float X, float Y, float RDeg, int S,int U) {
        super(X, Y);
        rotDeg = RDeg;
        xSpeed = 0.F;
        ySpeed = 10.F;
    ...
    public void step() {
        ...
        if (Main.elements.containsKey(user)) {
            ...
            this.rotDeg = wrk.rot;
            this.startX = wrk.x + 28.f * Utils.cos((int) this.rotDeg);
            this.startY = wrk.y + 28.f * Utils.sin((int) this.rotDeg);
