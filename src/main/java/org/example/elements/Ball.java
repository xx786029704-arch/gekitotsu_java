...
public class Ball extends Round {       //兵玉基类
    ...
    public void step(){     //照搬原版unit_func()
        ...
        if (hp <= 0 || game.hp0_flg[on_side] > 0 && jump_flg == 0) {
            new HitsDrop(game, this.x, this.y, game.unit[side]);
    ...
    public boolean land(){      //正常
        if (jump_flg == 0 && !(side == on_side) && game.hp0_flg[on_side] > 0) {
            jump_flg = 1;
            xs = game.seeder.random() * 7 - 3;
            ys = game.seeder.random() * 4 - 10;
            on_side = 1 - on_side;
        }
        cnt++;
        if (game.bases[on_side] != null) {
            move(game.bases[on_side].base_move_x, game.bases[on_side].base_move_y);
        }
        ys = ys + 1;
        drop_y = y + ys;
        if (drop_y >= 566) {
    ...
    public void snipe(){
        int wrk_target = (int) Math.round(Math.toDegrees(Math.atan2(game.core_y[1 - side] - y, game.core_x[1 - side] - x)));
