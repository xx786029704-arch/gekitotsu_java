...
public class SniperBall extends Ball {   //狙玉
    public SniperBall(Game game, float X, float Y, int R, int S, int TYPE) {   //初始化
    ...
    public void stepEx() {   //攻击逻辑
        float tRot = (float) Math.toDegrees(Math.atan2(this.game.core_y[1-side] - this.y, this.game.core_x[1-side] - this.x));
        ...
        if (this.cnt == this.speed) {
            ...
            new SniperBullet(this.game, spawnX, spawnY, this.side, cos_rot * 10F, sin_rot * 10F);
        }
    }
}
