...
public class MinigunBall extends Ball {   //机玉
    public MinigunBall(Game game, float X, float Y, int R, int S, int TYPE) {   //初始化
    ...
    public void stepEx() {   //攻击逻辑
        if (this.cnt >= this.speed && this.cnt % 4 == 0 && this.jump_flg != 1) {
            float vx = cos_rot * 22;
            float vy = sin_rot * 22;
            new GunBullet(this.game, this.x + vx * 3, this.y + vy * 3, this.side).setVec(vx, vy).setPower(22F);
        }
